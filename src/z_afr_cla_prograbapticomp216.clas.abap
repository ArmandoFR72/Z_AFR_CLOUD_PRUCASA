CLASS z_afr_cla_prograbapticomp216 DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS Z_AFR_CLA_PROGRABAPTICOMP216 IMPLEMENTATION.
...
* Example 1: Table Expression with Key Access
    out->write(  '--------------------------------------------' ).
    out->write(  'Example 1: Table Expressions with Key Access Prog16' ).
...
* Example 2: LOOP with key access
    out->write(  `-------------------------------` ).
    out->write(  `Example 2: LOOP with Key Access Prog16` ).
