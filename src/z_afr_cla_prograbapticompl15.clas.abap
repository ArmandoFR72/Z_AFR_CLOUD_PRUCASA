CLASS z_afr_cla_prograbapticompl15 DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS Z_AFR_CLA_PROGRABAPTICOMPL15 IMPLEMENTATION.
...
*    connection-carrier_name     = 'My Airline'.
    ...
    out->write(  '--------------------------------' ).
    out->write(  'Example 1: APPEND with Work Area Prog15' ).
...
* Example 2: APPEND with VALUE #( ) expression
    ...
    out->write(  '----------------------------' ).
    out->write(  'Example 2: Append with VALUE - Prog15' ).
    out->write(  connections ).
**********************************************************************
* Example 3: Filling an Internal Table with Several Rows
    ...
    out->write(  '-----------------------------------------' ).
    out->write(  'Example 3: Fill Internal Table with VALUE - Prog15' ).
    out->write(  carriers ).
**********************************************************************
* Example 4: Filling one Internal Table from Another
    connections = CORRESPONDING #( carriers ).
    out->write(  '--------------------------------------------' ).
    out->write(  'Example 4: CORRESPONDING for Internal Tables - Prog15 ' ).
